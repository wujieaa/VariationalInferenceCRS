...
class BeerPosDataModule2(pl.LightningDataModule):
    '''
    my_model3 data module
    '''
    def __init__(self, *, train_df,
        ...
        self.train_data_file_name = 'beer_train_data_for_bvae2.npy'
        self.user_grouped_data_file_name = 'beer_user_group_df_for_bvae2.pkl'
        self.args = args
    ...
    def prepare_data(self, *args, **kwargs):
        ...
        self.test_dataset = ValidDataset(self.test_df, user_grouped_df, self.word_num, self.args)
    ...
    def gen_data(self):
        ...
        train_item_series = self.train_df[[USER_INDEX, ITEM_INDEX]] \
            .groupby(USER_INDEX)[ITEM_INDEX].apply(list).rename(TRAIN_POS_ITEM_LIST)
        def compute_p_user(keys):
            count = np.zeros(self.word_num, dtype=np.int)
            for key in keys:
                v=np.array(self.sparse(key))
                count=count+v
            total=keys.shape[0]
            p=count*1.0/total
            min_val = 0.0001
            ...
            p[p < min_val] = min_val
            return p
        train_key_series = self.train_df[[USER_INDEX, KEY_VECTOR]] \
            .groupby(USER_INDEX)[KEY_VECTOR].apply(compute_p_user).rename(TRAIN_POS_P_LIST)
        ...
        user_items_df = pd.concat([all_series, train_item_series,train_key_series], axis=1)
        ...
        user_items_df[LEFT_NEG_ITEM_LIST] = \
            user_items_df.apply(lambda x: np.setdiff1d(all_items,
                                                       np.concatenate(
                                                           [x[ALL_POS_ITEM_LIST], x[TRAIN_NEG_ITEM_LIST]])).tolist(),
                                axis=1)
    ...
    def gen_train_data(self,user_grouped_df):
        pos_train_df = self.train_df[[USER_INDEX, ITEM_INDEX, KEY_VECTOR]]
        # pos_train_df[KEY_SPARSE] = pos_train_df[KEY_VECTOR].apply(self.sparse)
        pos_train_df[TRAIN_POS_P_LIST] = pos_train_df[USER_INDEX]\
            .apply(lambda user_index:user_grouped_df.loc[user_index,TRAIN_POS_P_LIST])
        train_data = pos_train_df[[USER_INDEX, ITEM_INDEX]].values
        print('KEY_SPARSE to Series')
        # train_vec = pos_train_df[KEY_SPARSE].apply(pd.Series).values
        train_p = pos_train_df[TRAIN_POS_P_LIST].apply(pd.Series).values
        print('concat')
        train_data = np.concatenate([train_data, train_p], axis=1)
        return train_data
...
class ValidDataset(Dataset):
    def __init__(self, pos_test_df, user_grouped_df, word_num, args):
        neg_grouped_df = user_grouped_df[[TRAIN_NEG_ITEM_LIST,LEFT_NEG_ITEM_LIST]]
        train_neg_grouped_s=user_grouped_df[TRAIN_NEG_ITEM_LIST]
        left_neg_grouped_s=user_grouped_df[LEFT_NEG_ITEM_LIST]
        self.pos_test_df = pos_test_df
        self.left_neg_grouped_df = left_neg_grouped_s.reset_index()
        self.train_neg_grouped_df=train_neg_grouped_s.reset_index()
    ...
    def reset_user_indexes(self):
        self.user_indexes = np.random.choice(self.left_neg_grouped_df.shape[0], self.len, replace=False)
    ...
    def __getitem__(self, index):
        ...
        user_left_neg_s = self.left_neg_grouped_df.iloc[user_index, :]
        user_train_neg_s = self.train_neg_grouped_df.iloc[user_index, :]

        return self.gen_single_data(user_left_neg_s,user_train_neg_s)
    ...
    def gen_single_data(self,user_left_neg_s, user_train_neg_s):
        user_index = user_left_neg_s[USER_INDEX]
        user_left_neg_items = user_left_neg_s[LEFT_NEG_ITEM_LIST]
        user_train_neg_items=user_train_neg_s[TRAIN_NEG_ITEM_LIST]
        ...
        user_left_neg_items = np.array(user_left_neg_items, dtype=np.int)
        user_train_neg_items=np.array(user_train_neg_items,dtype=np.int)
        user_neg_items=np.concatenate([user_left_neg_items,user_train_neg_items],axis=0)
        user_neg_items = np.random.choice(user_neg_items, self.args.valid_item_num_per_user - user_pos_items.shape[0],
                                          replace=False)
