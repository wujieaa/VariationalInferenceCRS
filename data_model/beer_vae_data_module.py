...
class BeerVaeDataModule(pl.LightningDataModule):

    def __init__(self, *, batch_size: int = 32):
        super().__init__()
        self.batch_size=batch_size
        train_item=BeerVaeDataModule.to_tensor(path.join(data_dir, 'Rtrain.npz'))
        train_keyphrase=BeerVaeDataModule.to_tensor(path.join(data_dir, 'Rkeyphrasetrain.npz'))

        test_item=BeerVaeDataModule.to_tensor(path.join(data_dir, 'Rtest.npz'))
        test_keyphrase = BeerVaeDataModule.to_tensor(path.join(data_dir, 'Rkeyphrasetest.npz'))
        ...
        self.critiquing_valid_dataset=BeerCritiquingValidDataset(train_item,num_sampled_users,5)
...
class BeerTrainDataset(Dataset):
    ...
    def __getitem__(self, index):
        items=self.item_data[index].view(-1)
        keyphrases=self.keyphrase_data[index].view(-1)
...
class BeerGeneralValidDataset(Dataset):
    ...
    def __getitem__(self, index):
        train_item=self.train_item_data[index].view(-1)
        train_keyphrase=self.train_keyphrase_data[index].view(-1)
        test_item = self.test_item_data[index].view(-1)
        test_keyphrase = self.test_keyphrase_data[index].view(-1)
        return torch.cat((train_item,train_keyphrase,test_item,test_keyphrase),dim=0)
    def __len__(self):
        return self.len
class BeerCritiquingValidDataset(Dataset):
    def __init__(self, train_item_data,  num_users_sampled, repeat=3):
        self.train_item_data = train_item_data
    ...
    def __getitem__(self, index):
        ...
        train_item=self.train_item_data[user_index].view(-1)
        #train_keyphrase=self.train_keyphrase_data[user_index].view(-1)
        return torch.cat((torch.tensor([user_index]),train_item),dim=0)
    def __len__(self):
        return self.len
