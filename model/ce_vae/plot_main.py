...
def main():
    result_path = path.abspath('../../../../lightning_logs/result.pkl')
    if (path.exists(result_path)):
        ...
        return
    item_result=data[CEVAELightning.FINAL_OUTPUTS_ITEM]
    base_dir=r'D:\code\lightning_logs\cevae'
    ndcgs=list(map(lambda x: x['NDCG'], item_result))
    plot1(ndcgs,title='ndcg',legend=['ndcg'],save_path=path.join(base_dir,'ndcg.png'))
    R_Precision = list(map(lambda x: x['R-Precision'], item_result))
    plot1(R_Precision, title='R-Precision', legend=['R-Precision'], save_path=path.join(base_dir,'R-Precision.png'))
    clicks = list(map(lambda x: x['Clicks'], item_result))
    ...
    atk.append(topk)
    for k in atk:
        recall = list(map(lambda x: x[f'Recall@{k}'], item_result))
        plot1(recall, title=f'Recall@{k}', legend=[f'Recall@{k}'], save_path=path.join(base_dir,f'Recall@{k}.png'))
        precision = list(map(lambda x: x[f'Precision@{k}'], item_result))
        plot1(precision, title=f'Precision@{k}', legend=[f'Precision@{k}'], save_path=path.join(base_dir, f'Precision@{k}.png'))
        map_arr = list(map(lambda x: x[f'MAP@{k}'], item_result))
        plot1(map_arr, title=f'MAP@{k}', legend=[f'MAP@{k}'], save_path=path.join(base_dir, f'MAP@{k}.png'))
    #-----------explanation------------
    explaination_result = data[CEVAELightning.FINAL_OUTPUTS_EXPLAINATION]
    recall = list(map(lambda x: x[f'Recall@{topk_keyphrase}'], explaination_result))
    ...
    precision = list(map(lambda x: x[f'Precision@{topk_keyphrase}'], explaination_result))
