...
from model.my_model5.ce_vi import CEVI
...
class CEVILightning(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        prior = batch[:, 3:]
        rating_scores, mean,logvar = self.model(user, item)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        rating, keyphrase,_= self.model(user, item, is_training=False)
    ...
    def configure_optimizers(self):
        ...
        scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[5,50, 100], gamma=0.1)
    ...
    def loss_function(self, *, rating_scores, rating_label,mean,logvar, prior):
        rating_scores = rating_scores.view(-1)
        rating_loss = F.binary_cross_entropy_with_logits(rating_scores, rating_label)
        kl_loss = torch.mean(0.5 * torch.sum(torch.square(mean-prior) + torch.exp(logvar) - logvar - 1, dim=1), dim=0)
        loss = self.args.rating_weight * rating_loss + self.args.keyphrase_weight * kl_loss
        return loss, rating_loss, kl_loss
